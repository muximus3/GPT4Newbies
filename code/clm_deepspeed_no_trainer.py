...
def train(accelerator, config: TrainArgs):
    ...
    with accelerator.main_process_first():
        train_dataset, val_dataset = load_tokenized_dataset(tokenizer, 
    ...
    if config.lora:
        target_modules =  config.lora_target_modules if config.lora_target_modules  else None
    ...
    accelerator.print(f"Len of train dataset: {dataset_size}")
    total_batch_size = config.micro_batch_size * accelerator.num_processes * gradient_accumulation_steps
    steps_per_epoch = dataset_size // total_batch_size 
    ...
    accelerator.print(f"Steps per batch: {steps_per_epoch}, Total training steps: {total_num_steps}")
    accelerator.print(f"Total batch size: {total_batch_size}, gradient_acc_steps:{gradient_accumulation_steps}")
